...
def create_kt_config_from_server_args(
    ...
    Args:
        server_args: Global server arguments
        layer_idx: Layer index in the model
        is_nextn: Whether this is a NextN (MTP) draft model layer.
                  When True, bypasses the DISABLE_KT_EP_WRAPPER guard and
                  creates an all-CPU mask for MTP expert layers.
    ...
    # MTP (NextN) layers are exempt from this guard — they use CPU experts.
    if not is_nextn:
    ...
    if is_nextn:
        # MTP layer: shift by num_hidden_layers so each draft layer gets a
        # unique index (e.g. mtp.0 → 43, mtp.1 → 44, …).  The kt-kernel
        # weight loaders detect ``layer_idx >= num_layers`` and switch the
        # key prefix from ``blk.{N}`` to ``mtp.{M}`` (zero-based MTP index).
